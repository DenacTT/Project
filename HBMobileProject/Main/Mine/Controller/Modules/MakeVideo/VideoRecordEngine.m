...
@interface VideoRecordEngine ()<AVCaptureVideoDataOutputSampleBufferDelegate,AVCaptureAudioDataOutputSampleBufferDelegate, CAAnimationDelegate>
{
    CMTime _timeOffset;     //录制的偏移CMTime
    CMTime _lastVideoTime;  //记录上一次视频数据文件的CMTime
    CMTime _lastAudioTime;  //记录上一次音频数据文件的CMTime
    
    NSInteger _cx;          //视频分辨率宽 1280
    NSInteger _cy;          //视频分辨率高 720
    int _channels;          //音频通道
    Float64 _samplerate;    //音频采样率
}

@property (strong, nonatomic) VideoRecordEncoder         *recordEncoder;    //录制编码
...
@implementation VideoRecordEngine
    ...
    @synchronized(self) {
        if (self.isCapturing) {
            ...
            dispatch_async(_captureQueue, ^{
                [self.recordEncoder finishedRecordWithCompletionHandler:^{
                    self.isCapturing = NO;
                    self.recordEncoder = nil;
                    self.startTime = CMTimeMake(0, 0);
...
- (void) captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    @synchronized(self) {
        ...
        if ((self.recordEncoder == nil) && !isVideo) {
            ...
            self.recordEncoder = [VideoRecordEncoder encoderForPath:self.videoPath videoHeight:_cy videoWidth:_cx audioChannel:_channels audioSamples:_samplerate];
        }
        //判断是否中断录制过
        if (self.discont) {
            if (isVideo) {
            ...
            CMTime last = isVideo ? _lastVideoTime : _lastAudioTime;
