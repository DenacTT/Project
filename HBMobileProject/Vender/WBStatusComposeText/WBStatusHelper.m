...
+ (NSString *)stringWithTimelineDate:(NSDate *)date {
    ...
    dispatch_once(&onceToken, ^{
        ...
        [formatterYesterday setDateFormat:STR(@"WBFH_yesterday")];
    ...
    if (delta < -60 * 10) { // 本地时间有问题
        ...
        return STR(@"WBFH_justNow");
    } else if (delta < 60 * 60) { // 1小时内
        return [NSString stringWithFormat:STR(@"WBFH_minuteAgo"), (int)(delta / 60.0)];
    } else if (date.isToday) {
        return [NSString stringWithFormat:STR(@"WBFH_hourAgo"), (int)(delta / 60.0 / 60.0)];
...
+ (NSString *)shortedNumberDesc:(NSUInteger)number {
    ...
    if (number <= 9999999) return [NSString stringWithFormat:STR(@"WBFH_Myriad"), (int)(number / 10000)];
    return [NSString stringWithFormat:STR(@"WBFH_TenMillion"), (int)(number / 10000000)];
}
