...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    _state.hasTapAction = _modelTapAction != nil;
    _state.hasLongPressAction = _textLongPressAction != nil;
    
    if (_highlight || _textTapAction || _textLongPressAction||_modelTapAction) {
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_state.trackingTouch) {
        ...
        if (!_state.touchMoved && _modelTapAction) {
            NSRange range = NSMakeRange(NSNotFound, 0);
            CGRect rect = CGRectNull;
            CGPoint point = [self _convertPointToLayout:_touchBeganPoint];
            YYTextRange *textRange = [self._innerLayout textRangeAtPoint:point];
            ...
            if (_highlight.model) {
                _modelTapAction(self, _innerText, range, rect, _highlight.model);
            }
        }
